...
import java.io.*;
import java.net.*;
class MyFrame extends JFrame implements ActionListener,KeyListener,Runnable{
		...
		if(e.getKeyCode()==e.VK_ENTER) {
			try {
				String msg = jtf.getText();
					jtf.setText("");
					jta.append("To  : "+msg+"\n");
					send_dp = new DatagramPacket(msg.getBytes(),msg.getBytes().length,ia,12345);
					ds.send(send_dp);
				}catch(Exception ee) {}
...
public class udp01 {
	...
	public static void main(String[] args) {
		MyFrame mf = new MyFrame("Chatting");
